            ...
            timer1 = Timer.scheduledTimer(withTimeInterval: 2, repeats: true) {_ in
                ...
                let task = URLSession.shared.dataTask(with: request) {(data, response, error) in
                    ...
                    if let data = data, let dataString = String(data: data, encoding: .utf8) {
                        ...
                        if self.Game == "Game:1" {
                            scanner1.scanUpToCharacters(from: self.sign1, into: &self.Player3)
                            scanner1.scanUpToCharacters(from: self.sign1, into: &self.Player3)
                            scanner1.scanUpToCharacters(from: self.sign1, into: &self.Player3)
                            if self.Player3 == "db>Player3:0"{
                              print("lap 1")
                                self.PostType = true
                                self.check = true
                                self.RandomColor()
                            ...
                            else if self.Player3 == "db>Player3:1"{
                                print("lap 2")
                                self.TimerFunc = false
                                self.timer2.invalidate()
                                self.check = false
                            }
                            else if self.Player3 == "db>Player3:2"{
                                print("reset")
