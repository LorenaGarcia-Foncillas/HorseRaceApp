...
class Player1ViewController: UIViewController {
    ...
    //SYNCHRONOUS FUNCTION: SEND GET REQUEST EVERY 2 SECS
            ...
            timer1 = Timer.scheduledTimer(withTimeInterval: 2, repeats: true) {_ in
                ...
                let task2 = URLSession.shared.dataTask(with: request2) {(data, response, error) in
                    ...
                    if let data = data, let dataString = String(data: data, encoding: .utf8) {
                        ...
                        if self.Game == "Game:1" {
                            scanner1.scanUpToCharacters(from: self.sign1, into: &self.Player1)
                            if self.Player1 == "br>Player1:0"{
                                print("lap 1")
                                self.PostType = true
                                self.check = true
                                self.RandomColor()
                            ...
                            else if self.Player1 == "br>Player1:2"{
                                print("reset")
    ...
    func RandomColor(){
        ...
        if number == 1 {
            Left_button.backgroundColor = colors[0]
        }
        else {
            Right_button.backgroundColor = colors[0]
        }
    }
